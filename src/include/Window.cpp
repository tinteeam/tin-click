...
HRESULT GameApp::Initialize() {
    ...
    if (SUCCEEDED(hr))
    {
        ...
        wcex.lpfnWndProc = GameApp::WndProc;
        wcex.cbClsExtra = 0;
...
HRESULT GameApp::CreateDeviceIndependentResources() {
...
HRESULT GameApp::CreateDeviceResources() {
    HRESULT hr = S_OK;
...
HRESULT GameApp::CreateGridPatternBrush(
...
void GameApp::DiscardDeviceResources()
{
    SafeRelease(&m_pRenderTarget);
    SafeRelease(&m_pBitmap);
    SafeRelease(&m_pBlackBrush);
...
LRESULT CALLBACK GameApp::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (message == WM_CREATE)
    {
        ...
        GameApp* pGameApp = (GameApp*)pcs->lpCreateParams;
    ...
    {
        GameApp* pDemoApp = reinterpret_cast<GameApp*>(
            ::GetWindowLongPtrW(
                hwnd,
                GWLP_USERDATA
            ));
...
HRESULT GameApp::LoadResourceBitmap(
    ID2D1RenderTarget* pRenderTarget,
...
HRESULT GameApp::LoadBitmapFromFile(
    ID2D1RenderTarget* pRenderTarget,
    IWICImagingFactory* pIWICFactory,
    PCWSTR uri,
    UINT destinationWidth,
